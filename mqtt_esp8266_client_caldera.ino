...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1') {
    ...
    client.publish("casa/caldera/control", "1");//publico un 1 para indicar que se ha encencido la caldera
  } else {
    digitalWrite(ledPin, HIGH);  // Apagamos la caldera con High porque el hardware es de logica negada
    client.publish("casa/caldera/control", "0");//publico un 0 para indicar que se ha apagado la caldera
  }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish("casa/caldera/control", "Bienvenido al control de la caldera to wapo hermano");
      // ... and resubscribe en el topic casa/caldera(la raspberry enviara las ordenes)
      client.subscribe("casa/caldera");
    } else {
